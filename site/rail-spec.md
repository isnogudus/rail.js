...
- Catch structural errors at **check time**, not at run time.
...
- **Cycles in the wire graph are allowed** and used for retry,
  poll, and iteration patterns — see §7.4 and §9.13.
...
- `check()` — validates the node and its sub-nodes for
  execution. Idempotent; safe to call multiple times. No-op if
  already checked.
- `isChecked(): boolean` — returns `true` iff the node is currently
  in a valid, ready-to-execute state.
- `invoke(name, ctx, runState, local)` — **internal** method
  called by the library runner (and by Activity/Parallel-Node
  implementations on their sub-nodes). Not part of the user
  API; user code does not call it. See "Invoke contract" below.
...
**Signature.** `invoke(name, ctx, runState, local)`
...
- `runState` — the internal per-run record (§6.1). Carries step
  ...
  `killSignal`, logger, top-level flow name, the cycle-counter
  map, and the local-state map. Implementations may read and
  modify it (e.g. increment depth around inner runs). Not
  exposed to step functions; steps see only `runInfo` and a
  `local` parameter (§4).
- `local: Object` — the position-local state for this node's
  position, as retrieved by the calling step-execution loop
  from `runState.shared.localState`. Always an object; the
  caller substitutes `{}` if no local has been stored for the
  position yet. The implementation passes it to user code (in
  the case of Step-Nodes) and may propagate an updated value
  via the return.

**Return value (success).** Either a value of shape
`{ output, ctx?, local? }` or a `Promise` that resolves to such
a value.
...
- `local?: Object` — optional. If present, the calling
  step-execution loop stores it under this position's path in
  `runState.shared.localState`, replacing whatever was there.
  If absent, the stored local is unchanged. Step-Nodes propagate
  this from their user function; Activities and Parallel-Nodes
  do not write local in v1 (they always return without a
  `local` field).

**Why `local` is in the invoke contract for all node kinds.**
Activities and Parallel-Nodes in v1 do not produce a `local`
write; their invoke returns never include a `local` field, and
their own `local` parameter goes unused inside `invoke`. The
parameter is in the signature anyway, for uniformity: every
node kind has the same invoke shape, and the runner's step-loop
treats every position identically (read local, pass in, store
returned local if any). The tracer-event consequence is that
`activity-enter`/`-leave` and `branch-start`/`-end` events
carry a `local` field that, in v1, equals `{}` (or whatever an
earlier mechanism may have stored at that position's path,
which no current built-in does). This is conceptual noise the
spec accepts in exchange for invoke-contract uniformity, and it
leaves the door open for future node kinds that *do* want to
expose a local at the Activity or Parallel-Node level without
changing the contract.

**Failure.** `invoke` throws (or its returned Promise rejects):

- A `RailRuntimeError` or `RailCheckError` propagates as-is to
  the calling runner.
...
**Preconditions.** `isChecked()` must be `true`. Otherwise `invoke`
...
`Activity.check()` validates the graph (three phases — see §7),
recursively checks sub-nodes (whether `'step'`, `'activity'`,
or `'parallel'`) by calling their `check()` if `isChecked()` is
false, and builds the runtime adjacency lookup.
`Activity.isChecked()` reflects whether validation has succeeded
since the last modification. `Activity.invoke(name, ctx, runState, local)`
...
`railKind`, `check()`, or `invoke()`. It is created with the
...
- `name: string` — the top-level name, used for logging and
  diagnostics.
- `node` — the held node.
- **`run(ctx, opts)`** — runs `node.check()` first if the held
  node's `isChecked()` is `false`, then allocates a fresh
  run-state from `opts`, invokes the held node with the top-level
  name, and returns the `RunResult`. If `check()` fails, the
  resulting `RailCheckError` propagates out of `run(...)` before
  any step executes. Subsequent calls to `run(...)` on the same
  flow re-use the already-checked state — `check()` is idempotent.
  See `flow.run(...)` below for full details.
...
// def.check() is optional — sendMessageFlow.run(...) runs it
...
name is a check error, not a duplicate handle.
...
run-state — and passes it to the step as a third parameter
(after `ctx` and `local`). In v1, run-info contains `signal`,
`input`, `invocation`, and `path`. See §4 for the contract and
...
- Runtime: `flow`.
- Errors: `RailCheckError`, `RailRuntimeError`, `RailBuildError`.
...
const sendMessageFlow = flow('sendMessage', sendMessage);   // check() runs on first .run()
...
- `activity(...)` returns an Activity (railKind `'activity'`) in
  the **unchecked** state. Call `.check()` to validate it and
  prepare it for execution; `.isChecked()` returns `false` until
  `.check()` has succeeded.
...
- `fn` — a step function
  ...
  (§4). Steps that don't need `local` or `runInfo` can omit the
  trailing parameters.
- `options.inputs?: string[]` — default `['in']`. Declaring more
  ...
  `nodeHandle.in('portName')` (see §3.4). Each name must satisfy
  the name rules (§3.3) eagerly; structural validation that
  `inputs` is non-empty and free of duplicates happens at
  `check()` time (`EMPTY_INPUTS`, `DUPLICATE_INPUT` in §7.2).
- `options.outputs: string[]` — declared output port names. Each
  name must satisfy the name rules (§3.3); validation that
  `outputs` is non-empty and free of duplicates happens at
  `check()` time (`EMPTY_OUTPUTS`, `DUPLICATE_OUTPUT` in §7.2),
  not at `node(...)` factory time. The factory itself does not
  raise on structural problems with `outputs` — it accepts the
  options shape and defers structural validation to `check()`,
  consistent with how Activities defer post-builder validation.
  The exception is per-name validation (`INVALID_NAME` for
  reserved characters or empty strings), which is eager.
...
The builder maintains, internally and incrementally, the
activity's data structures: a node registry (name → node), an
entry/exit registry, a wire index keyed by source endpoint
(§11), and a set of already-wired output ports. Each builder
method consults and updates these structures, raising
`RailBuildError` synchronously when a rule is violated. The
stack trace therefore points at the exact builder call that
introduced the problem.

- **`a.entry(name)`** — declares the activity's single entry.
  Returns an entry handle. `name` must satisfy the name rules
  (§3.3, *Name restrictions*); otherwise `INVALID_NAME`.
  Calling `entry(...)` twice raises `MULTIPLE_ENTRIES`.
- **`a.exit(name)`** — declares an exit. Returns an exit handle.
  `name` must satisfy the name rules; otherwise `INVALID_NAME`.
  Declaring two exits with the same name raises `DUPLICATE_EXIT`.
...
- **`a.addNode(name, node)`** — adds a Rail-Node to the activity
  ...
  - `INVALID_NAME` if `name` violates the name rules.
  - `NOT_A_NODE` if `node` fails `isRailNode(...)`.
  - `DUPLICATE_NODE` if a node with the same `name` was already
    added.
  ...
  separate handle; they share the same checked implementation.

- **`a.wire(source, target)`** — declares a wire from source
  handle to target handle. Updates the wire index and the
  set of already-wired output ports. Performs structural checks
  ...
  - `MULTIPLE_OUTGOING_WIRES` — the source output port already
    has a wire. Each output port may have at most one outgoing
    wire (multiple incoming wires to the same input are allowed;
    see §7.5).
  - `MULTIPLE_ENTRY_WIRES` — `source` is the entry handle and
    a wire from the entry has already been declared.
  - `NO_ENTRY` — `source` is an entry handle but the activity
    has no entry yet (i.e. the caller passed in an entry handle
    from somewhere else — this is mostly a defensive check, since
    `entry(...)` returns the only legitimate entry handle).

  These checks happen at the wire-call site so the stack trace
  points directly to the offending line. Other validation
  (completeness, topology) waits for `check()`.
...
Every user-supplied name in the library — node names assigned via
`a.addNode(name, ...)`, port names declared in a step-node's
`inputs` and `outputs`, entry names from `a.entry(name)`, exit
names from `a.exit(name)`, branch keys in `parallel(...)`, and
the top-level flow name passed to `flow(name, ...)` — must
satisfy the following rules:

- Must be a **non-empty string**.
- Must not consist only of whitespace.
- Must not contain the reserved characters **`.`** or **`:`**.

The two reserved characters are used internally for trace-path
notation (sub-activity paths like `fan.branchA.validate`, §8.4)
and for endpoint identification in the wire index (§11). Allowing
them in user-supplied names would make those notations ambiguous.

Violations raise `RailBuildError(INVALID_NAME)` synchronously at
the call site (the factory, the builder method, or the
`flow(...)` call), so the stack trace points to the offending
declaration.
...
```js
{
  ...
  check():     void,              // validate + recursively check sub-nodes
  isChecked(): boolean,
  invoke(name, ctx, runState, local):
                                  // internal; called by the library runner.
                                  // Returns { output, ctx?, local? } per the
                                  // invoke contract (§2). For an Activity,
                                  // `output` is the name of the exit reached.
                                  // Throws on unchecked state or runtime error.
...
`isChecked()` flips from false to true on successful `check()`
and stays true. The model permits future editing APIs that would
flip `isChecked()` back to false on mutation — this spec is
...
- `name: string` — the top-level diagnostic name. Used by the
  default logger and in error messages. Must satisfy the name
  rules in §3.3 (non-empty, no whitespace-only, no `.` or `:`);
  otherwise `RailBuildError(INVALID_NAME)`.
...
- The flow holds the node by reference and does not modify it.
  The factory itself does **not** invoke `check()` — see
  `flow.run(...)` below.
...
- **Ensures the held node is checked.** If `node.isChecked()` is
  `false`, `run` calls `node.check()` first. The check is
  recursive (§7), so sub-activities and parallel branches are
  checked too. If checking raises `RailCheckError`, that error
  propagates out of `run(...)` before any step executes. After
  this step, the held node and all its sub-nodes satisfy
  `isChecked() === true` for this run and every subsequent run.
  Because `check()` is idempotent, subsequent runs of the same
  flow re-use the already-checked state (no work).
- Allocates a fresh run-state from `opts` (see §6.1). The
  `local` argument passed to the top-level invoke is `{}` (the
  initial value for the top-level position).
- Invokes `node.invoke(name, ctx, runState, local)`, where
  `name` is the flow's top-level name. The invoke returns
  `{ output, ctx?, local? }` (§2 invoke contract); the `local`
  field of the return, if any, is stored in `localState` for the
  top-level position like any other position's local.
...
TraceEntry = {
  ...
  invocation: number,         // 1-based count of how often this position has been
                              // entered in this run; see §6.2. Same value as
                              // runInfo.invocation for the corresponding step.
  local:      Object,         // snapshot of the position-local state after the
                              // step. If the step returned a new local, that
                              // new value; otherwise the value passed in. On
                              // throw, the incoming value (since no update was
                              // stored). Always an object; `{}` if nothing has
                              // ever been stored at this position.
...
- `branches`: object whose keys are branch names and whose values
  ...
  `node(...)` if needed. Each branch key must satisfy the name
  rules (§3.3) eagerly; the factory raises
  `RailBuildError(INVALID_NAME)` immediately for invalid keys.
  Structural validation that the branches map is non-empty
  happens at `check()` time (`EMPTY_PARALLEL` in §7.2).
...
- `outputs: ['done']` — fixed.
- `inputs: ['in']` — fixed.
- `check()` — validates the branches map (`EMPTY_PARALLEL` if
  empty), then recursively calls `branch.check()` on each branch
  whose `isChecked()` is false. Errors raised during sub-check
  are nested into a `RailCheckError` with a path hint identifying
  the branch.
- `invoke(name, ctx, runState, local)` — runs the branches via
...
If a branch raises a `RailRuntimeError` or `RailCheckError`
...
check during the outer activity's `check()` reaches them and
their branches. Branch-level structural errors thus surface at
outer-check time, not at runtime.
...
- `outputs` = `stepNode.outputs ∪ Object.values(mapping)`,
  ...
  insertion order).
- An `invoke(name, ctx, runState, local)` that runs `stepNode.invoke(...)`
  and, on a thrown exception, first checks the error class:
  - **`RailRuntimeError` and `RailCheckError` are never
...
**`check()` and `isChecked()`** delegate to `stepNode`. The
wrapper holds a reference; checking the wrapper checks the
...
// baseSend is shared and checked exactly once.
```

The shared inner step is checked once and reused; only the
...
```
StepReturn =
  | string                                  // shorthand: produces this output;
                                            // ctx and local unchanged
  | { output: string,
      ctx?:   Object,                       // if present, replaces the running ctx;
                                            // if absent, the running ctx is unchanged
      local?: Object }                      // if present, replaces the stored local
                                            // for this position; if absent, the
                                            // stored local is unchanged
```

The step function receives **three parameters**, all positional:

1. **`ctx`** — the running ctx flowing through the workflow. The
   step may read from it and, by returning a `ctx` field,
   replace it for the downstream of the run.
2. **`local`** — the position-local workspace for this step's
   position. Always an object (the library pre-initialises it to
   `{}` for the first invocation at a position). The step reads
   from it and, by returning a `local` field, replaces it for
   later invocations at the same position. See §6.1 for storage
   semantics and §9.13 for the canonical retry pattern.
3. **`runInfo`** — library-injected metadata about the current
   invocation: cancellation signal, activated input port,
   invocation count, full path. Read-only as far as the library
   is concerned; the step cannot change it.

The first two parameters (`ctx`, `local`) are the **programmer's
two slots**: read as parameters, written via `StepReturn`. The
third (`runInfo`) is the library's slot: pure observation /
metadata. The mechanical symmetry is intentional.

Steps that don't need the trailing parameters can omit them
entirely:

```js
async function plain(ctx) { /* ... */ }                  // OK
async function withLocal(ctx, local) { /* ... */ }       // OK
async function full(ctx, local, runInfo) { /* ... */ }   // OK
```

The `local = {}` and `runInfo = {}` defaults in the formal
signature ensure that a step called by hand without the trailing
arguments behaves correctly.
...
RunInfo = {
  ...
  invocation: number,         // 1-based count of how often this position has
                              // been entered in the current run; see §6.2
  path:       string,         // full dotted path of this position
                              // (e.g. 'parent.sub.op'); see §6.1
}
```

`runInfo.invocation` is provided for **observability** —
logger output, trace records, tracer-driven UIs. It is **not**
intended as a control-flow input for the step: it reflects the
run-global view of how often this position has been entered,
which does not necessarily match a step's intent for retry
budgets or iteration counters (e.g. position re-entry via a
sub-activity loop also increments it). For state that drives
the step's own decisions, use the `local` parameter; for state
that should reset at activity boundaries, route it through
`ctx`. See §9.13.
...
```js
async function fetchPayload(ctx, local, runInfo) {
...
```js
async function recover(ctx, local, runInfo) {
...
`runInfo.invocation` is a 1-based count of how often the
**current position** in the flow has been entered during this
run. The "position" is identified by its full path (the dotted
local name within all enclosing scopes — same path that appears
as the trace `step` field, §8.4). On the first entry,
`invocation` is `1`; on the second `2`; and so on. Position
identity is by path, **not** by node instance: if the same step
or sub-activity is added under two different local names in the
same outer activity, each position has its own independent
counter, both starting at `1`. If the position is entered again
via a wire that loops back to it, its counter increments.

For control-flow patterns like retry budgets, use `local`
(§9.13) rather than `runInfo.invocation`: `local` is under the
step's control (it persists only what the step explicitly
returns), while `invocation` is run-global telemetry. A
sub-activity loop, for example, would re-enter the inner steps
and increment their `invocation` counters across all
sub-activity calls — which is correct as observation but
usually not what a retry budget wants.

`runInfo.path` is the same full dotted path that
`runInfo.invocation` is keyed by — e.g. `'fan.branchA.validate'`
for the `validate` step inside the `branchA` branch of the `fan`
parallel-node. Useful for diagnostic output and structured
logging. Steps that don't care can ignore the field.
...
The `local` parameter gives every node position a private
workspace that survives across invocations of the same position.
The library:

- Pre-initialises `local` to `{}` on the first invocation, so
  steps can read fields with the `??` operator without any
  defensive check.
- Stores updates only when the step explicitly returns a `local`
  field. Mutating `local` inside the step has no effect — it
  follows the same "in-as-parameter, out-as-return-value"
  pattern as `ctx`.
- Keys the stored value by the full path of the position (same
  path used for `cycleCounters` and trace `step` fields). The
  same node instance used at two different positions has two
  independent locals.
- Persists the value for the rest of the run by default. To
  reset on sub-activity boundaries, route the relevant state
  through `ctx` instead — the wire that re-invokes the
  sub-activity controls the initial ctx, giving you an explicit
  reset point.

Example — retry pattern (full version in §9.13):

```js
async function tryOp(ctx, local) {
...
Each invocation reads the previous `tries` count, increments it,
and writes it back via `local`. The position-local state is
preserved between invocations of the loop without any
library-level "max-cycle" machinery.
...
propagates out of `run()`. A `RailCheckError` raised inside a
...
| `NOT_A_NODE`               | A non-node value was passed where a Rail-Node was required (e.g. `a.addNode(name, value)` or `flow(name, value)`). |
| `INVALID_NAME`             | A node, port, entry, exit, branch key, or flow name was empty, whitespace-only, or contained a reserved character (`.`, `:`). See §3.3. |
...
| `NO_ENTRY`                 | An `a.wire(...)` referenced an entry handle, but no `a.entry(...)` has been declared yet.           |
| `MULTIPLE_ENTRIES`         | `a.entry(...)` was called more than once on the same builder.                                       |
| `DUPLICATE_EXIT`           | `a.exit(...)` was called twice with the same name.                                                  |
| `DUPLICATE_NODE`           | `a.addNode(...)` was called twice with the same name.                                               |
| `MULTIPLE_OUTGOING_WIRES`  | `a.wire(srcOut, ...)` was called for an output port that already has a wire.                        |
| `MULTIPLE_ENTRY_WIRES`     | `a.wire(entryHandle, ...)` was called more than once.                                               |

These catch errors at the earliest possible moment — at the
calling line, with a stack trace pointing to it — rather than
during `check()` or runtime. See §7.1 for the full description.
...
runState = {
  ...
  path:         string,    // path prefix to the current scope, used to key
                           // cycleCounters; '' at top level
  ...
  shared: {
    ...
    flowName:       string,
    cycleCounters:  Map<string, number>,  // key: full path to a position
                                          // (see §6.2); value: number of
                                          // invocations so far. Shared
                                          // across all forks of this run.
    localState:     Map<string, Object>,  // key: full path to a position;
                                          // value: position-local state, last
                                          // written by that position's invoke.
                                          // Absent key means "no local stored
                                          // yet" — runner substitutes {} when
                                          // reading. Shared across all forks.
  }
}
```
...
`fork.depth = runState.depth + 1` and `fork.path = <new path>`
for sub-activities). Because JavaScript spreads primitive values
by copy and object values by reference, this single line
achieves the right semantics: per-fork slots become independent,
the shared sub-object stays shared.
...
not a per-branch budget) and so that `cycleCounters` reflects
the run as a whole. Without the layered design, two parallel
Activity-branches sharing a single run-state would trample each
other's `depth`, `currentInput`, and `path`, producing wrong
values in trace entries, tracer events, and cycle counters.
...
with `depth + 1` and a new `path` (the parent's `path` extended
by the sub-node's local name), runs the inner loop with the
fork, and discards the fork on return. The outer's run-state is
therefore unmodified — there is no decrement-on-return, and no
special handling for throw paths (the fork simply goes out of
scope). Each `TraceEntry` records the per-fork depth that was
active when the step executed (§6.2).

**Path construction.** The `path` slot accumulates the full
trace path from the run's root down to the current scope. At
the top level it is `''`. When the step-execution loop is about
to invoke a sub-node named `n`:

- The **full path** for this invocation is computed as
  `path === '' ? n : path + '.' + n`.
- For sub-activity and parallel-branch forks, that full path
  becomes the fork's new `path`. Inside that fork, further
  sub-node names will be appended on top.
- For step-node invocations, the full path is used as the
  `cycleCounters` key and as the `step` field of the
  `TraceEntry`. Step-nodes do not fork, so the outer's `path`
  is unchanged.

The same dotted format is used for trace `step` names (§8.4),
for `cycleCounters` keys, and for tracer-event `step` fields —
one unified path notation throughout the runtime.

The run-state is internal: it is not visible to step
implementations. When a step is invoked, the runner constructs a
**run-info** object —
`{ signal: shared.combinedSignal, input: currentInput,
   invocation: <next>, path: <fullPath> }` — and
passes it to the step as the third argument, alongside the
position-local `local` as the second argument (§4). The
run-info is the step-visible projection of the run-state.
...
5. **Compute the full path, update the cycle counter, and read
   local:**
   - `fullPath = runState.path === '' ? name : runState.path + '.' + name`
     where `name` is the sub-node's local name in this scope.
   - `invocation = (runState.shared.cycleCounters.get(fullPath) ?? 0) + 1`
   - `runState.shared.cycleCounters.set(fullPath, invocation)`
   - `local = runState.shared.localState.get(fullPath) ?? {}`
   - The `fullPath` is used as the `step` field in the TraceEntry
     and tracer events; `invocation` is exposed to the step via
     `runInfo` and recorded in the trace; `local` is passed into
     `invoke` as the fourth argument.
6. Execute the node:
   - Capture `t0 = now()`.
   - Emit a `step-start` tracer event with `step` (= `fullPath`),
     `depth` (= `runState.depth`), `input` (= `runState.currentInput`),
     `invocation`, `local` (the incoming value just read), and
     `kind` (= `node.railKind`).
   - Call `node.invoke(name, ctx, runState, local)`, where `name`
     is the node's local name (assigned at `addNode` time). The
     call returns `{ output, ctx?, local? }` (§2 invoke contract)
     or throws. Each kind implements `invoke` differently:
     - A **Step-Node** constructs `runInfo = { signal:
       runState.shared.combinedSignal, input:
       runState.currentInput, invocation, path: fullPath }`,
       calls the user function as `fn(ctx, local, runInfo)`, and
       translates the `StepReturn` (§4) into the contract's
       `{ output, ctx?, local? }` shape. If the user function
       returns a `local` field, it is propagated upward through
       the invoke return; if absent, the invoke return has no
       `local` field.
     - An **Activity** forks the run-state with `depth + 1` and
       `path = fullPath`, emits an `activity-enter` event
       (carrying the inner depth, `invocation`, and the
       incoming `local`), runs its inner step-execution loop
       with the fork. On normal termination (an exit reached),
       it emits `activity-leave` (carrying the *outer* depth and
       outgoing local — same as incoming, since v1 Activities do
       not write their own local) and returns
       `{ output: <exitName>, ctx: <innerCtx> }` (no `local`
       field). If the inner loop propagates a `RailRuntimeError`
       or `RailCheckError`, the activity instead emits
       `activity-throw` (with the propagating error, outer
       depth, and the incoming local) and re-throws. The outer's
       `runState.depth` and `runState.path` are unchanged in
       either case.
     - A **Parallel-Node** forks the run-state once per branch
       (each branch gets an independent fork with `path =
       fullPath + '.' + branchKey` and its own per-fork slots,
       all sharing the same `runState.shared`), emits a
       `branch-start` event before each branch's invocation
       (carrying the branch's own incoming local, which is
       read from `localState` at path `fullPath + '.' +
       branchKey`), runs all branches via `Promise.allSettled`.
       For each settled branch, it emits either `branch-end`
       (on success, with the branch's terminal output and its
       outgoing local) or `branch-throw` (if the branch
       propagated an error, with the propagating
       `RailRuntimeError`/`RailCheckError`). After all branches
       settle, it returns `{ output: 'done', ctx:
       <parallel-results-ctx> }` (no `local` field) if all
       ...
       its inner forks will carry `depth + 1`. Each branch's
       `path` extends the Parallel-Node's path with the branch
       key, so branch-local cycle counters and locals are keyed
       at `fan.branchA.*` versus `fan.branchB.*` and never
       collide.
   ...
   - If the returned object has a `ctx` field, replace the
     ...
     unchanged.
   - **Store local.** If the returned object has a `local` field,
     write it to `runState.shared.localState.set(fullPath, ...)`,
     replacing any previously stored value. If the `local` field
     is absent, the stored value (if any) is unchanged. The
     library never deletes from `localState`; the canonical
     "reset" is `{ local: {} }`, semantically equivalent to "no
     entry stored" since reads substitute `{}` for missing keys.
   - Compute `duration = now() - t0`.
   - Emit a `step-end` tracer event with `step` (= `fullPath`),
     `depth`, `output`, `duration`, `invocation`, `local` (the
     outgoing value: the newly-stored value if the return had a
     `local` field, otherwise the same value that was passed in),
     and `kind`.
   - Append a `TraceEntry` to the trace with the current
     `runState.depth`, `step` (= `fullPath`), `invocation`, and
     `local` (the outgoing value, same as in `step-end`), then
     call the logger.
...
emits a `step-throw` tracer event (with the original error,
duration, and the **incoming** local — the value that was passed
into `invoke`, since a thrown invocation cannot return a new
one), classifies the thrown value (§6.4), and either re-raises a
`RailRuntimeError`/`RailCheckError` as-is or wraps any other
exception into `RailRuntimeError(UNHANDLED_THROW)`. The trace
entry for the throwing step is appended with `threw: true`,
`output: null`, and the incoming `local` before propagation. The
stored local in `runState.shared.localState` is **not** updated
on throw: the canonical "in-as-parameter, out-as-return-value"
rule means a throw produces no return, hence no update. The next
invocation at the same position (if any) reads the same stored
value the failing one did.

**Intentional asymmetry: `invocation` vs. `local` on throw.**
The `invocation` counter is incremented in step 5, **before**
the step runs — so failed attempts count toward `invocation`.
The `local` write happens **after** the step returns — so failed
attempts do not produce a `local` update. This asymmetry is
deliberate and reinforces the doctrine "`invocation` is
telemetry, `local` is control state": telemetry counts every
attempt the runner made (a failed attempt is still an attempt
from the run's point of view), while control state only reflects
what the step explicitly decided to record. A step that wants
to count failures in its control state must do so inside its
own logic and emit an updated `local` along with whatever output
it chooses (e.g. catching the throw inside the step and
returning `{ output: 'retry', local: { failures: ... } }`).
...
`{ output, ctx?, local? }` on success. How that shape is
produced differs per kind:
...
- String return `'foo'`: invoke result is `{ output: 'foo' }`.
  The runner preserves the running ctx and the stored local.
- Object return `{ output, ctx?, local? }`: forwarded as-is. If
  `ctx` is present, the runner replaces the running ctx; if
  absent, the runner leaves it unchanged. If `local` is present,
  the runner stores it under the position's full path; if
  absent, the stored local is unchanged.
...
1. **`RailRuntimeError` or `RailCheckError`:** re-thrown
...
by two spaces per level of `entry.depth`. When `entry.invocation
> 1` (i.e. the position has been entered more than once in this
run, typically via a loop), the line is suffixed with ` #N`
where `N` is the invocation count:
...
[rail:retry]       OK   op               (12.04ms) -> retry
[rail:retry]       OK   op               (15.42ms) -> retry #2
[rail:retry]       OK   op               (14.83ms) -> ok #3
...
deeper. The `#N` suffix appears only when `invocation > 1`, so
linear workflows are not visually noisy with `#1` on every line.
...
exposes it to step implementations as `runInfo.signal` (a field
of the third parameter to step functions, §4). The library
performs no action on this signal itself; steps decide how to
react.
...
- A step polls between sub-operations:
  ...
  ```js
  async function process(ctx, local, runInfo) {
...
`inner.invoke(name, ctx, runState, local)` (§8.3) — passing through the
...
TracerEvent =
  ...
  | { type: 'step-start',
      ...
      invocation: number,             // 1-based; see §6.2
      local: Object,                  // incoming local for this invocation
  ...
  | { type: 'step-end',
      ...
      invocation: number,
      local: Object,                  // outgoing local: new value if returned,
                                      // else same as incoming
      kind: 'step' | 'activity' | 'parallel' }
  ...
  | { type: 'step-throw',
      ...
      duration: number,
      invocation: number,
      local: Object,                  // incoming local (no update on throw)
      kind: 'step' | 'activity' | 'parallel' }
  ...
  | { type: 'activity-enter',
      ...
      invocation: number,             // 1-based count for this activity position
      local: Object }                 // incoming local
  ...
  | { type: 'activity-leave',
      ...
      invocation: number,
      local: Object }                 // outgoing local (= incoming in v1; activities
                                      // do not write their own local)

  | { type: 'activity-throw',
      ts: number,
      depth: number,                  // outer depth (the fork is gone)
      name: string,
      error: Error,                   // the propagating RailRuntimeError or RailCheckError
  ...
  | { type: 'branch-start',
      ...
      invocation: number,             // 1-based count for this branch position
      local: Object }                 // incoming local for this branch position
...
- For **Activities and Parallel-Nodes used as nodes** (`kind:
  ...
  `RailRuntimeError`/`RailCheckError` before it propagates out
...
`RailRuntimeError`/`RailCheckError` actually **propagating
...
- The tracer **may** start new flow runs during event handling,
  ...
  fully separate. Checking already-checked nodes during event
  handling is also safe (check is idempotent).
...
The library validates an activity's topology in **two distinct
moments**:

1. **During the builder closure** (eager validation, raises
   `RailBuildError` synchronously at the offending call site).
2. **In `check()`** (post-builder validation, raises
   `RailCheckError`).

The first moment catches everything that can be detected as
declarations and wires are added one by one. The second moment
catches everything that can only be known after the whole
activity is assembled — completeness and topology.

### 7.1 Eager validation in the builder

While `activity(builderFn)`'s closure is running, every builder
method performs synchronous validation against the activity's
in-progress data structures. The activity carries — internally,
across builder calls — a node registry, an entry/exit registry,
a wire index keyed by source endpoint (§11), and a set of
already-wired output ports. Each builder method consults and
updates these structures, raising `RailBuildError` with the
appropriate code if a rule is violated. The library does not
defer these checks; they happen exactly when the offending line
runs, so the stack trace points to it.

`RailBuildError` codes raised during build:

| Code                       | When                                                                                                          |
|----------------------------|---------------------------------------------------------------------------------------------------------------|
| `UNKNOWN_PORT`             | `nodeHandle.out('xyz')` or `.in('xyz')` references a port not declared on the node.                           |
| `NOT_A_NODE`               | `a.addNode(name, value)` or `flow(name, value)` was given a value where `isRailNode(value)` is false.         |
| `INVALID_NAME`             | Any user-supplied name (node, port, entry, exit, branch key, flow) was empty, whitespace-only, or contained a reserved character (`.`, `:`). See §3.3. |
| `INVALID_WIRE_DIRECTION`   | `a.wire(src, tgt)` source is not usable as a source, or target is not usable as a target.                     |
| `AMBIGUOUS_NODE_INPUT`     | `a.wire(src, nodeHandle)` where the node has multiple inputs; use `nodeHandle.in('port')` instead.            |
| `WIRE_FROM_OTHER_BUILDER`  | `a.wire(...)` was given a handle returned by a different activity's builder.                                  |
| `CATCHING_REQUIRES_STEP`   | `catching(node, mapping)` was called with a node whose `railKind !== 'step'`.                                 |
...
Build errors are precise: each raises immediately with the line
information that triggered it. Programmers see the failing line
of their builder closure in the stack trace.

### 7.2 `check()` — post-builder validation

After the builder closure returns, the activity holds a complete
internal data structure but has **not yet** been validated for
completeness and topology. The flag `isChecked()` is `false`.

`Activity.check()` validates these remaining concerns. It runs
in **two phases**; errors within a phase are collected and
reported together. If a phase produces errors, the next phase
is not executed.

```js
class RailCheckError extends Error {
  name = 'RailCheckError';
  phase: 'completeness' | 'topology';
  errors: CheckIssue[];
}
...
`check()` is **idempotent**: if the activity is already in the
checked state (`isChecked() === true`), it returns immediately
without redoing work. Otherwise it proceeds through the two
phases. On successful completion, `isChecked()` flips to `true`
and stays `true`.

Before its own two phases, `check()` recursively invokes
`node.check()` on every sub-node held in the activity's node
registry. Each sub-node's `check()` is itself idempotent, so
this is safe to call regardless of whether the sub-node is
already checked. For Step-Nodes, `check()` validates the node's
own configuration (e.g. `EMPTY_OUTPUTS`, `EMPTY_INPUTS`). For
Activity sub-nodes, sub-check recursively descends into the
inner activity. For Parallel-Nodes, sub-check checks each branch.

If a sub-node's `check()` raises a `RailCheckError`, the outer
`check()` nests it as a sub-error with a path hint identifying
the nesting node, and proceeds to its own completeness phase.

This recursive-check-via-`check()` mechanism gives implicit,
identity-based memoisation: any node instance is checked at most
once across all its uses, because checking is idempotent.
Passing the same node instance to multiple `addNode` calls — or
using the same Activity in multiple outer activities — results
in checking happening exactly once.

#### Step-Node and Parallel-Node self-checks

A Step-Node's `check()` validates its own configuration. The
possible issues:

| Code                | Meaning                                                     | Fields            |
|---------------------|-------------------------------------------------------------|-------------------|
| `EMPTY_OUTPUTS`     | The step was declared with empty `outputs`.                 | —                 |
| `DUPLICATE_OUTPUT`  | Same output name listed twice in the step's `outputs`.      | `output`          |
| `EMPTY_INPUTS`      | The step was declared with empty `inputs`.                  | —                 |
| `DUPLICATE_INPUT`   | Same input name listed twice in the step's `inputs`.        | `input`           |

A Parallel-Node's `check()` checks each branch (recursively) and
validates that the branches map is non-empty:

| Code               | Meaning                                                  | Fields           |
|--------------------|----------------------------------------------------------|------------------|
| `EMPTY_PARALLEL`   | `parallel({})` was called with no branches.              | —                |

These step- and parallel-self-checks could be eagerly performed
in the factory; they are deferred to `check()` so that the
factory itself never throws structural errors — it returns a
node object regardless, and structural problems surface at
check time alongside the activity's own.

### 7.3 Phase 1 — completeness

After all sub-nodes have been checked, the activity verifies its
own completeness:

| Code                       | Meaning                                                  | Fields                |
|----------------------------|----------------------------------------------------------|-----------------------|
| `NO_ENTRY`                 | No `a.entry(...)` was declared in this activity.         | —                     |
| `NO_EXITS`                 | No exits were declared in this activity.                 | —                     |
...
is **not** an error. Such an input is dead topology — it can
never be activated at runtime — but the asymmetry to
...
Note: completeness does not include "no `addNode` was ever
called" — an activity with only an entry directly wired to an
exit is a perfectly valid pass-through, and is allowed.

### 7.4 Phase 2 — topology

Once completeness holds, the activity verifies topology:

| Code                       | Meaning                                                                                                | Fields            |
|----------------------------|--------------------------------------------------------------------------------------------------------|-------------------|
| `UNREACHABLE_NODE`         | No wire path from entry to this node.                                                                  | `node`            |
| `UNREACHABLE_EXIT`         | No wire path from entry to this exit.                                                                  | `exit`            |
| `NO_EXIT_PATH`             | This node has no wire path forward to any exit; every path leaving it loops back without reaching one. | `node`            |

Cycles in the wire graph are **valid**. Activities may declare
loops freely — e.g. for retry, polling, or iteration patterns
(see §9.13). Topological well-formedness is the conjunction of
two independent reachability properties:

1. **Forward from the entry:** every declared node and exit must
   be reachable from the entry. A simple BFS from the entry,
   following wires in their normal direction, identifies the
   reachable set. Anything not in it raises `UNREACHABLE_NODE`
   (for nodes) or `UNREACHABLE_EXIT` (for exits).
2. **Backward from the exits:** from every node, there must be
   a wire path to at least one exit. A reverse BFS starting
   from all declared exits, following wires in the reverse
   direction, identifies the set of "exit-reachable" nodes.
   Any forward-reachable node that is **not** in this set
   raises `NO_EXIT_PATH` — it is in a structurally trapped
   region of the graph from which no exit can be reached
   regardless of which outputs the step-logic chooses.

Both walks are linear in the number of nodes plus wires. The
two together establish: every node can be entered from the entry
and can in principle reach an exit. **Whether a particular run
actually reaches an exit depends on the step-logic** and is
beyond the library's static analysis — the `maxSteps` budget
(§6.1) is the runtime backstop. A successful `check()` therefore
guarantees structural well-formedness, not termination on every
trajectory through the graph.

The `NO_EXIT_PATH` check catches the canonical structural
mistake: a strongly-connected region of nodes whose every
outgoing wire stays within the region. Such a region traps any
run that enters it; the run can only terminate via
`STEP_LIMIT_EXCEEDED`. Forcing the structural check at
`check()`-time gives that diagnosis early.
...
## 8. Sub-activities and recursive check
...
same `check()`/`isChecked()`/`invoke()` interface.
...
### 8.2 Check and runtime behavior

When `outer.check()` runs, it invokes `check()` on every
sub-node, which in turn recursively checks inner nodes. The
operation terminates because check is idempotent: any node
already checked returns immediately.

When the runner reaches a sub-activity node during execution, it
calls `inner.invoke('inner', ctx, runState, local)` — using the
local name assigned at `addNode` time and the local read from
`localState` for the sub-activity's position — and shares the
outer run-state. The step counter, signals, logger, cycle
counters, and local state are all common to both. A kill
...
it returns `{ output, ctx?, local? }` or throws.
...
- On failure, the sub-activity throws a `RailRuntimeError` or
  `RailCheckError`; the error propagates unchanged (§5). A
...
Because check is idempotent and recursive, **the same node
instance is checked exactly once**, no matter how many places
reference it. The memoisation is implicit and automatic; there is
no cache, no flag-tracking gymnastics, no separate
"checked-version" object — just the node's own `isChecked()`
flag.

Example:

```js
// 1) Define an inner activity. Not checked yet.
const inner = activity((a) => {
  const s  = a.entry('in');
  const ok = a.exit('ok');
...
outer1.check();
// inner was checked exactly once during outer1.check().
...
outer2.check();
// inner.check() is called again here, but is a no-op
// (inner.isChecked() is already true).
```
...
// Optional explicit check — outerFlow.run(...) below would also
// run it on first invocation. Demonstrates that check() is
// idempotent and recursive: calling it on `outer` checks
// `inner` too, but the same `inner` would not be re-checked if
// it had been checked before.
outer.check();
const outerFlow = flow('outer', outer);   // factory does not check
// outerFlow.run(...) below will see isChecked()===true and skip.
```

### 9.4 Check error example
...
// recursively checks the Parallel-Node, which recursively
// checks profileActivity and keysActivity.
...
or more activities. It is checked exactly once.
...
flowA.check();   // checks validateNode once
flowB.check();   // validateNode already checked — no-op
```
...
const typoFlow = flow('typo', def);   // check() runs on first .run()
...
### 9.13 Retry pattern with `local`

Cycles are valid in the wire graph (§7.4). The step uses its
`local` parameter to track retry attempts:
...
const retryFlow = activity((a) => {
  ...
  a.wire(start,            op);
  a.wire(op.out('retry'),  op);          // ← loops back, valid
  a.wire(op.out('ok'),     success);
  a.wire(op.out('giveup'), failure);
});

const main = flow('retry', retryFlow);
const result = await main.run({ url: '...' });
```

`check()` accepts this topology: `op` is reachable from the
entry, and from `op` both `success` and `failure` are reachable
via the `ok` and `giveup` outputs. The cycle on `retry` is
allowed; no `NO_EXIT_PATH` error because exits *can* be reached
— whether they actually are depends on the step.

On the first entry into `op`, `local` is `{}` (the library
pre-initialises it). The step computes `tries = (local.tries ?? 0)
+ 1 = 1`, decides to call the unreliable backend, and returns
with `local: { tries: 1 }`. The library stores that value under
the path `op` in `runState.shared.localState`. On the second
entry (via the `retry` loop), the step reads `local.tries === 1`,
increments to `2`, and so on. After three attempts, the step
returns `'giveup'`. The retry budget is fully under the step's
control — there is no library-level "max-cycle" mechanism, only
the `maxSteps` backstop for genuinely unbounded loops.

**Why `local`, not `runInfo.invocation`.** Both fields would
count `1`, `2`, `3` for this simple top-level case. They diverge
when the retry node is reached repeatedly via an outer loop or
re-entered through a sub-activity wrapper: `runInfo.invocation`
keeps incrementing globally across the whole run, while `local`
is whatever the step last wrote. For control flow ("how many
attempts have I made in *this* burst of retries"), `local` is
what you want; `runInfo.invocation` is for observability
(logging, tracing, diagnostics).

**Reset behaviour.** Because `local` is keyed by full path and
shared across the run, two distinct positions of the same node
(e.g. added under names `first` and `second`) have independent
locals — each starts fresh at `{}`. To reset within a single
position (e.g. "start a fresh retry burst when re-entering this
sub-activity"), route the relevant state through `ctx` instead:
the wire that re-invokes the sub-activity then carries an
explicit reset.

**Trace and tracer.** The trace entries and `step-end` events
for each `op` invocation carry the outgoing `local` snapshot:

```
trace[0] = { step: 'op', output: 'retry', ..., invocation: 1, local: { tries: 1 } }
trace[1] = { step: 'op', output: 'retry', ..., invocation: 2, local: { tries: 2 } }
trace[2] = { step: 'op', output: 'ok',    ..., invocation: 3, local: { tries: 3 } }
```

The default logger highlights repeated invocations with a
`#N` suffix (§6.6):
...
A polling pattern works the same way: replace `'retry'` with
`'wait'` and call `setTimeout(...)` (with `runInfo.signal`) in
the step before returning. An adaptive backoff can use
`local.tries` (or any field the step writes into `local`) to
compute its delay.
...
rail.js               // public API: activity, node, parallel, merge, catching,
                      ...
                      //   RailCheckError, RailRuntimeError, RailBuildError
...
- **Endpoint handles are frozen objects.** `Object.freeze` after
  ...
  `a.wire(...)`.
- **Build-time wire index.** The activity's wire index (see also
  the "Wire indexing" note above) is constructed **incrementally**
  during the builder closure, inside `a.wire(src, tgt)`. There
  is no separate "build the lookup table" step in `check()` —
  the index is already complete when the closure returns.
  `check()` only reads it (for completeness and topology
  phases). This is the central simplification of the build/check
  split.
- **Topology phase implementation.** Phase 2 runs two BFS walks
  over the activity's wire index:
  - Forward BFS from the entry, following wires in their normal
    direction. Yields the forward-reachable set. Any declared
    node not in it raises `UNREACHABLE_NODE`; any declared exit
    not in it raises `UNREACHABLE_EXIT`.
  - Reverse BFS starting from all declared exits, following
    wires in the reverse direction (target → source). Yields
    the exit-reachable set. Any forward-reachable node not in
    this set raises `NO_EXIT_PATH`.
  Both walks are O(V + E) and need no SCC analysis. The wire
  index built at build time is the input to both walks.
- **`cycleCounters` lifecycle.** A single `Map<string, number>`
  is allocated on `shared` at run start. It is never replaced
  during the run, never forked. The step-execution loop reads
  and updates it directly. Keys are full dotted paths
  constructed from each scope's `path` slot extended by the
  sub-node's local name. Because the `path` slot is per-fork,
  parallel branches that descend into the same Activity instance
  produce different key prefixes (e.g. `fan.branchA.x` vs
  `fan.branchB.x`) — collisions are structurally impossible.
- **`localState` lifecycle.** A single `Map<string, Object>` is
  allocated on `shared` at run start. Storage is **lazy**: the
  map only gets an entry for a position once that position's
  invoke returns a `StepReturn` whose object form includes a
  `local` field. Steps that never write `local` produce no
  entries — the runtime cost for non-using steps is one
  `Map.get` returning `undefined` per invocation. Reads
  substitute `{}` for missing entries so the step never sees
  `undefined`. Keys use the same full-path construction as
  `cycleCounters`, so the same path-disambiguation applies
  (parallel branches and lineary-reused node instances get
  independent entries by construction). The map is never
  forked and never reset during the run; the canonical reset
  is `{ local: {} }`, which is semantically identical to "no
  entry stored" (since reads substitute `{}` anyway).
- **Check status.** Each Node carries an internal flag. Its
  `check()` consults the flag at the top: if already checked,
  return immediately. Otherwise it validates, performs any
  kind-specific structural validation (e.g. for Step-Nodes:
  `EMPTY_OUTPUTS`, `DUPLICATE_OUTPUT`, etc.), recursively
  checks children (Activity → sub-nodes; Parallel-Node →
  branches), and sets the flag at the end. `isChecked()`
...
- **Sort identification.** A value is a Rail-Node iff
  ...
  semantics differ across kinds. Generic operations (`check`,
  `isChecked`, `invoke`) rely on the uniform interface.
- **Step-Node implementation.** Internally a Step-Node is an
  object with `railKind: 'step'`, `inputs`, `outputs`, an
  internal `_fn`, an internal `_checked` flag, and the three
  Node methods. Constructed by `node(fn, opts)`.
  `invoke(name, ctx, runState, local)` constructs the run-info
  object `{ signal: runState.shared.combinedSignal, input:
  runState.currentInput, invocation, path }`, calls
  `_fn(ctx, local, runInfo)`, and translates the user function's
  `StepReturn` (§4) into the invoke-contract shape:
  - String return `'foo'` → `{ output: 'foo' }` (no `ctx`, no
    `local`; stored ctx and local stay unchanged).
  - Object return `{ output, ctx?, local? }` → forwarded as-is;
    the calling step-execution loop will store the `local` field
    if present.
...
- **Activity implementation.** An object with
  ...
  endpoints, sub-nodes, wires), `_checked` flag, lookup table
  built during check, and the Node methods plus `toMermaid`.
  ...
  interface. `invoke(name, ctx, runState, local)` increments depth, runs
...
- **Parallel-Node implementation.** An object with
  ...
  `branches` map, `_checked` flag, and the Node methods.
  `check()` recursively checks each branch.
  `invoke(name, ctx, runState, local)` uses `Promise.allSettled` with
...
- **`catching(...)` implementation.** Validates that the input is
  ...
  inner step and to the mapping. Its `invoke(name, ctx, runState, local)`
  calls `inner.invoke(name, ctx, runState, local)` inside `try`/`catch`;
  on a thrown exception, looks up `e.name` in the mapping —
  `RailRuntimeError` and `RailCheckError` are re-thrown
  unchanged (they are graph errors, not domain failures), other
  exceptions are mapped if the name is in the mapping or
  re-thrown otherwise. `check()` and `isChecked()` delegate to
  the inner step.
...
- **Sub-node recognition during `addNode`.** `a.addNode(name,
  value)` calls `isRailNode(value)` synchronously. If the check
  fails, `addNode` raises `RailBuildError(NOT_A_NODE)` immediately,
  with a stack trace pointing at the builder call line. The
  failing value is **not** recorded in the node registry; the
  builder closure can continue but any further references to
  that name will not resolve. This is consistent with all other
  build-time eager checks (§7.1).
...
- **Wire indexing.** Internally, an activity's wires can be
  stored as a Map keyed by source-endpoint string, with
  target-endpoint strings as values. Endpoints follow a
  namespace-prefixed format:
  - `node:<nodeName>.<portName>` for a port on a node (used for
    both wire sources, i.e. outputs, and wire targets, i.e.
    inputs).
  - `entry:<entryName>` for an activity entry (wire source).
  - `exit:<exitName>` for an activity exit (wire target).

  The `:` separates the namespace from the local identifier;
  the `.` separates node name from port name within the `node:`
  namespace. Both characters are reserved in user-supplied names
  (§3.3), so the format is unambiguous. The resulting index gives
  O(1) wire lookups during the step-execution walk, O(1)
  uniqueness checks for outgoing wires from each output port
  (enforced eagerly by `a.wire(...)`'s `MULTIPLE_OUTGOING_WIRES`
  check), and `Set`-difference detection of unwired output
  ports during the completeness phase (§7.3). The choice of
  separators is an internal convention; the public API exposes
  only handle methods (`node.out(port)`, `node.in(port)`, etc.)
  and does not surface these strings.
- **Internal abort linkage in `parallel`.** The Parallel-Node
  ...
  a `RailRuntimeError`/`RailCheckError`, the internal
...
2. The example from §9.3 (sub-activity) runs, the outer's
   `check()` recursively checks the inner (if unchecked),
...
3. The example from §9.5 (parallel + evaluate) runs and produces
   ...
   activity) surface at `loadProfileAndKeys.check()` time, not
   at runtime.
...
6. The example from §9.8 (reuse under multiple names)
   demonstrates that `validateNode` is checked exactly once
...
8. The example from §9.10 demonstrates cooperative cancellation:
   ...
   delivered to step functions via `runInfo.signal` (a field of
   the third parameter, after `ctx` and `local`); it is not
   present on the ctx. A step that takes only `(ctx)` continues
   to work.
...
10. The `catching(stepNode, mapping)` helper produces a Step-Node
    ...
    `RailRuntimeError` and `RailCheckError` thrown from the
...
15. Calling `flow.run(...)` on a flow whose held `node.isChecked()`
    is `false` invokes `node.check()` before any step executes.
    If checking fails, the resulting `RailCheckError` propagates
    out of `run(...)`. If checking succeeds, the held node
    satisfies `isChecked() === true` and the run proceeds.
    Subsequent runs of the same flow re-use the already-checked
    state (no extra work).
16. Constructing `flow(name, value)` where
    `isRailNode(value)` is false raises
    `RailBuildError(NOT_A_NODE)`.
17. **Name validation.** All user-supplied names (node names in
    `a.addNode`, port names in `node({...inputs, outputs})`,
    entry names in `a.entry`, exit names in `a.exit`, branch
    keys in `parallel({...})`, and the top-level name in
    `flow(name, ...)`) raise `RailBuildError(INVALID_NAME)` at
    the call site when they are empty, whitespace-only, or
    contain a reserved character (`.` or `:`). The check is
    synchronous; the stack trace points to the offending line.
18. `addNode` rejects non-node values with
    `RailBuildError(NOT_A_NODE)` synchronously at the call site.
19. **Build-time eager errors.** The following raise
    `RailBuildError` at the offending builder call line, with
    the stack trace pointing to it: `MULTIPLE_ENTRIES`,
    `DUPLICATE_NODE`, `DUPLICATE_EXIT`,
    `MULTIPLE_OUTGOING_WIRES`, `MULTIPLE_ENTRY_WIRES`,
    `NO_ENTRY`, `UNKNOWN_PORT`, `INVALID_WIRE_DIRECTION`,
    `AMBIGUOUS_NODE_INPUT`, `WIRE_FROM_OTHER_BUILDER`,
    `CATCHING_REQUIRES_STEP`.
20. `Activity.check()` correctly reports each error code in §7.3
    (completeness) and §7.4 (topology) with a minimal failing
    definition. Errors within a phase are collected; phases
    short-circuit. Unwired node-inputs are **not** an error
    (asymmetric to `UNWIRED_OUTPUT`).
21. `check()` is idempotent: calling it multiple times on the
    same Activity is a no-op after the first successful call;
    `isChecked()` returns true after the first.
22. The same Activity instance passed to multiple `addNode` calls
    (in one or more outer activities) is checked exactly once,
    as demonstrated in §8.3 and §9.8.
23. Typed-ctx helpers behave correctly:
    ...
    - A ctx produced by `exceptionCtx(err, inputCtx)` satisfies
      `isExceptionCtx(...) === true`.
24. Kill switch: when `opts.killSignal` is aborted, the run
    rejects with `RailRuntimeError(KILLED)` before the next node
    starts. The error carries the trace and the flow name.
25. Sub-activity run-state sharing: a sub-activity's steps count
    toward the outer `maxSteps`; `killSignal` aborts both inner
    and outer; `runInfo.signal` is the same combined signal in
    both runs. `shared.cycleCounters` and `shared.localState` are
    also shared, so a step inside a sub-activity sees its
    `invocation` count and its `local` value preserved across
    repeated entries of the sub-activity (e.g. via a loop around
    the sub-activity position). `maxSteps` is run-global,
    including across parallel branches; the synchronous,
    single-threaded counter increment ensures deterministic
    enforcement.
26. Trace `depth` field is correctly populated: top-level steps
...
27. `flow.toMermaid()` for the §9.2 flow produces parseable
...
28. `flow.toMermaid()` for §9.3's outer renders the sub-activity
    node as a subroutine shape with class `subActivity`, not
    expanded.
29. `flow.toMermaid()` for §9.5 renders the Parallel-Node with a
    distinct shape and class `parallelNode`, not expanded.
30. `flow.toMermaid()` for §9.7 (top-level Step-Node) produces a
    minimal diagram: a synthetic entry, the step node, and one
    synthetic exit per declared output, with edges connecting
    them.
31. The default logger emits one line per executed step in
    ...
    any.
32. When `opts.tracer` is provided, the runner emits the events
    ...
    - **Activity:** `activity-enter` (inner depth) and either
      ...
      `RailRuntimeError`/`RailCheckError`, duration) on
      failure.
...
33. **Stateless flow and reentrancy.** A flow object created
...
34. **Per-fork run-state isolation.** When two Activity
...
35. **Naming inside parallel branches.** A trace entry produced
    ...
    keys.
36. **Cycles are valid topology.** The example in §9.13 (retry
    flow with a wire from `op.out('retry')` back to `op`) passes
    `check()` without error. The `CYCLE` code does not exist.
37. **`NO_EXIT_PATH` detection.** An activity where some
    forward-reachable node has no wire path to any exit (e.g. a
    closed cycle from which no output leaves the cycle) raises
    `RailCheckError` in Phase 2 with code `NO_EXIT_PATH` and a
    `node` field naming each trapped node. Detected by reverse
    BFS from the exits.
38. **`invocation` counter.** During a run, `runInfo.invocation`
    and `TraceEntry.invocation` reflect the 1-based count of how
    often the current position has been entered. The counter is
    keyed by the full dotted path (so the same node instance
    added at two different positions has independent counters,
    both starting at 1). Loops cause the counter to increment.
    `shared.cycleCounters` is observable via the trace and
    tracer events.
39. **Tracer events carry `invocation`.** Every `step-start`,
    `step-end`, `step-throw`, `activity-enter`, `activity-leave`,
    `activity-throw`, `branch-start`, `branch-end`, and
    `branch-throw` event includes an `invocation` field that
    matches the corresponding `runInfo.invocation` /
    `TraceEntry.invocation` value.
40. **Default logger and invocation.** The default logger
    appends ` #N` to lines where `invocation > 1`; for
    `invocation === 1`, no suffix is added.
41. **Step signature.** A Step-Node's user function is called
    with three positional parameters: `(ctx, local, runInfo)`.
    Functions that omit trailing parameters work without error;
    the library's formal signature defaults `local` to `{}` and
    `runInfo` to `{}`.
42. **`local` pre-initialisation.** On every Step-Node
    invocation, the library passes a `local` argument that is
    always an object — `{}` if no local has been stored at the
    position's full path, otherwise the previously-stored value.
    The step never has to test for `undefined`.
43. **`local` persistence on return.** If a step returns a
    `StepReturn` whose object form includes a `local` field, the
    library stores that value under the position's full path in
    `runState.shared.localState`, replacing any prior value. If
    the `local` field is absent (string-shorthand return, or
    object return without `local`), the stored value is
    unchanged. The library never deletes entries; `{ local: {} }`
    is the canonical reset.
44. **`local` is not stored on throw.** When a step throws
    (returns a rejected Promise or synchronously throws), the
    library does not write any new value to
    `localState[fullPath]`. The next invocation at the same
    position reads the same value the throwing one did.
45. **`local` independence by path.** The same node instance
    added under two different local names in the same outer
    activity (e.g. `first` and `second`) has two independent
    `local` entries, both pre-initialised to `{}` on first
    invocation. Linear reuse therefore behaves like two
    independent steps.
46. **`local` in trace and tracer events.** Every `TraceEntry`
    carries a `local` field with the outgoing value (post-step,
    or incoming if the step threw or did not return a `local`
    field). Every `step-start`, `activity-enter`, and
    `branch-start` event carries the incoming `local`; every
    `step-end`, `activity-leave`, and `branch-end` event carries
    the outgoing `local`; every `step-throw`, `activity-throw`,
    and `branch-throw` event carries the incoming `local` (since
    a throw produces no update).
47. The library has no runtime dependencies (`package.json` has
    no `dependencies`).
48. All public symbols are documented with JSDoc that includes
    types.
...
- Editing API: methods to add/remove nodes or wires after the
  builder closure has returned. The `isChecked()` flag is designed
...
- **Node** — abstract base concept for elements in a graph. A
  plain object with `railKind: string`, `check()`, `isChecked()`,
  `invoke(name, ctx, runState, local)`. Concrete kinds: Step-Node,
...
- **Check** — validates a node post-builder: completeness and
  topology phases for activities (§7.3, §7.4); structural
  configuration validation for step-nodes; recursive sub-check
  for activities and parallel-nodes. Sets `isChecked()` to true.
  Idempotent. Called automatically by `flow.run(...)` on the
  first run if `isChecked()` is false; callers may invoke
  `check()` explicitly for early validation.
- **Build-time eager validation** — checks performed inside
  builder methods (e.g. `a.wire(...)`, `a.addNode(...)`) the
  moment they are called. Raise `RailBuildError` synchronously
  with a stack trace pointing at the offending builder line.
  See §7.1.
...
- **Trace** — the ordered list of `TraceEntry` records produced
  ...
  `threw`, `invocation`, `local`, and an optional `error` field.
  The `depth` field counts sub-activity nesting (0 for top-level);
  `invocation` is the 1-based count of how often this position has
  been entered in the run; `local` is the snapshot of the
  position-local state after the step. See §3.6 and §6.2.
...
- **Run-info** — the **third** parameter passed to step
  functions: `(ctx, local, runInfo) => ...`. Carries
  library-side per-run information that is not domain data and
  cannot be written by the step. In v1:
  `{ signal?, input, invocation, path }` — the combined
  cancellation signal, the name of the activated input port,
  the 1-based count of how often the current position has been
  entered in this run, and the full dotted path of the current
  position. The shape is forward-stable; fields may be added in
  future versions but not removed. See §4.
- **Invocation count** — the 1-based number of times a position
  in the flow has been entered during the current run. Available
  to steps as `runInfo.invocation`, recorded in `TraceEntry` as
  `invocation`, and carried on every step/activity/branch
  tracer event. Position identity is by path (see "Path"), so
  the same node instance used at two different positions has two
  independent counters. Loops cause the counter to increment
  past `1`. Intended for **observability** (logger, trace,
  tracer-driven UIs) rather than control flow — for step-level
  state like retry counters, use `local` instead. See §6.2 and
  §9.13.
- **Path** — the dotted full name of a position in the flow,
  e.g. `'fan.branchA.validate'`. Used as the trace `step` field,
  the `cycleCounters` key, the tracer-event `step` field, and
  `runInfo.path`. Constructed by extending each enclosing
  scope's path with the local name. One notation, used
  consistently. See §6.1.
- **`cycleCounters`** — a `Map<string, number>` in the run-state's
  shared sub-object, keyed by full path, holding the
  invocation count for every position entered in this run.
  Shared across all forks; updated by the step-execution loop
  before each invoke. See §6.1.
- **Local** — the second parameter passed to step functions:
  `(ctx, local, runInfo) => ...`. A position-local workspace
  pre-initialised to `{}`, persisted across invocations of the
  same position when the step explicitly returns a new value via
  `{ local: ... }`. Symmetric to `ctx`: read as a parameter,
  written via the return value. Keyed by full path, so two
  positions referencing the same node instance have independent
  locals. See §4 and §9.13.
- **`localState`** — a `Map<string, Object>` in the run-state's
  shared sub-object, keyed by full path, holding the
  position-local state most recently returned by each position's
  invoke. Absent keys mean "no local stored yet" and reads
  substitute `{}`. Shared across all forks; updated only when a
  step's `StepReturn` includes a `local` field. See §6.1.
...
- **Run-state** — internal, per-run record structured in two
  layers (§6.1):
  - **Per-fork slots** (copied on fork by shallow spread):
    `depth` (sub-activity nesting), `currentInput` (the input
    port for the next Step-Node invoke), `path` (the dotted
    path prefix to the current scope).
  - **Shared sub-object** (held by reference; visible to all
    forks): `stepCounter`, `maxSteps`, `killSignal`,
    `combinedSignal`, `logger`, `tracer`, `flowName`,
    `cycleCounters` (Map of path → invocation count),
    `localState` (Map of path → position-local state).
  Allocated by `flow.run(...)` and threaded through every node
  invoked during the run; sub-activities and parallel branches
  receive forks. Not directly visible to step implementations;
  the run-info is the step-visible projection, and `local` is
