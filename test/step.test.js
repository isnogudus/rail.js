...
import { node, activity, flow, RailCheckError, RailRuntimeError } from '../rail.js';
...
describe('Step-Node (§3.2, §4)', () => {
  ...
  it('compile validates outputs (non-empty, no duplicates) and inputs', () => {
    ...
    expect(() => empty.check()).toThrow(RailCheckError);
    try { empty.check(); } catch (e) {
      expect(e.errors.some((x) => x.code === 'EMPTY_OUTPUTS')).toBe(true);
    }

    const dup = node(() => 'ok', { outputs: ['ok', 'ok'] });
    try { dup.check(); throw new Error('expected throw'); } catch (e) {
    ...
    try { dupIn.check(); throw new Error('expected throw'); } catch (e) {
      expect(e).toBeInstanceOf(RailCheckError);
  ...
  it('top-level Step-Node sees runInfo.input = first declared input (acceptance #9)', async () => {
    let captured;
    const s = node(
      (_ctx, _local, runInfo) => { captured = runInfo.input; return 'ok'; },
